...
class Embeddings(nn.Module):
    def __init__(self,
                 w2v,
                 args,word_emb_froze, weight = None):
    ...
    def forward(self, sent, mask=None):
        ...
        if self.use_att == 0:
            ...
            if len(sent.shape) == 3:
              ...
              att_weights = att_weights.reshape(sent.shape[0], sent.shape[1], sent.shape[2])
...
class Normalize(nn.Module):
    ...
    def forward(self, pro_pred):
        ...
        predict = torch.nn.functional.one_hot(tmp_pro, pro_pred.shape[-1]) # [bz, n_labels]
        # reli
        tmp_reli = pro_pred - torch.min(pro_pred, dim = -1).values.unsqueeze(1)
        tmp_reli = tmp_reli/((torch.max(tmp_reli, dim = -1).values.unsqueeze(1))+0.001)
