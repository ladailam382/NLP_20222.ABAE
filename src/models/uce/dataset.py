...
class TrainDataset(data.Dataset):
    def __init__(self,args, w2v):
        ...
        self.w2id = w2v.wv.key_to_index
        if "<pad>" in self.w2id:
            self.pad_idx = self.w2id['<pad>']
        else:
            self.pad_idx = self.w2id['<etc>']
    ...
    def load_file(self, file: str):
        ...
        for i in range(len(dt)):
            ...
            for sd in dt[i].split():
                if sd in self.w2v.wv.key_to_index:
                  num_max += 1
            if num_clus > num_max:
                num_clus = num_max
            s = dt[i].split()
    ...
    def sent2idx(self, tokens: List[str], is_sent):
        ...
        if is_sent: 
            tokens = [self.w2id[token.strip()]
                    for token in tokens if token in self.w2id.keys()]
    ...
    def __getitem__(self,id):
        ...
        return torch.LongTensor(sent), torch.LongTensor(seeds), torch.as_tensor(self.num_clusters), torch.as_tensor(self.num_arr), torch.as_tensor(np.array(self.mask).astype(int))#, torch.LongTensor(label), torch.tensor(reli)
...
class TestDataset(data.Dataset):
    def __init__(
        ...
        self.maxlen = args.maxlen
        self.maxlen_seed = args.maxlen_seed
        if "<pad>" in w2v.wv.key_to_index:
            self.pad_idx = w2v.wv.key_to_index['<pad>']
        else:
            self.pad_idx = w2v.wv.key_to_index['etc']
        self.w2id = {w: w2v.wv.key_to_index[w] for w in w2v.wv.key_to_index}
        self.general_idx = args.general_idx
        self.num_cluster_general = args.num_cluster_general
        self.num_cluster_aspects = args.num_cluster_aspects
        self.w2v = w2v
    ...
    def __getitem__(self,id):
        ...
        if len(self.test_data[id]) == 1:
            label = -1
        else:
            label = self.test_data[id][1]
        return torch.LongTensor(sent), torch.LongTensor(seeds), torch.as_tensor(label), torch.as_tensor(self.num_clusters), torch.as_tensor(self.num_arr), torch.as_tensor(np.array(self.mask).astype(int))