...
from src.models.uce.model.layers import Embeddings, SentencesEmbeddings, Normalize
import numpy as np
np.random.seed(0)
...
class Model(nn.Module):
    def __init__(self,args, w2v,
        ...
        self.word_emb_teacher = Embeddings(self.w2v, args, self.word_emb_froze_teacher, self.weight_teacher)
        self.word_emb_student = Embeddings(self.w2v, args, self.word_emb_froze_student, self.weight_student)
        self.sent_emb = SentencesEmbeddings(args,self.use_att_sent, self.w2v.wv.vector_size)
        self.sd_eb_teacher = SentencesEmbeddings(args,self.use_att_seed_teacher, self.w2v.wv.vector_size)
        self.sd_eb_student = SentencesEmbeddings(args,self.use_att_seed_student, self.w2v.wv.vector_size)
    ...
    def forward(self, sents, seeds,num_clusters,num_arr,mask, flag):
        ...
        pro_pred_teacher = torch.matmul(sd_emb_teacher,snt_emb_teacher_transpose).squeeze(-1)
        ...
        pro_pred_student = torch.matmul(sd_emb_student,snt_emb_student_transpose).squeeze(-1)
        ...
        for i in range(1, tmp.shape[0]):
            tmp_ = torch.max(pro_pred_student[:,torch.sum(tmp[:i]):torch.sum(tmp[:i+1])], -1).values.unsqueeze(-1)
            index = torch.argmax(pro_pred_student[:,torch.sum(tmp[:i]):torch.sum(tmp[:i+1])], -1).unsqueeze(-1)
        ...
        for i in range(1, tmp.shape[0]):
            tmp_ = torch.max(pro_pred_teacher[:,torch.sum(tmp[:i]):torch.sum(tmp[:i+1])], -1).values.unsqueeze(-1)
            pro_tmp = torch.cat((pro_tmp,tmp_), 1)
        ...
        if flag:
          ...
          for i in range(len(pro_pred_student)):
            ...
            softmax.append(lst)
          return torch.argmax(pro_pred_student,-1), pro_pred_student
    ...
    def my_loss(self, predict_teacher, pro_pred_student,reli_teacher):
        ...
        if self.number_loss == 0:
          loss = torch.mul((1+self.anpha*torch.abs(reli_teacher)),torch.mul(tmp_denta, tmp_denta)).sum() 
